...
const ProfileWalkerForm = ({ onSubmit, onReset, onFormChange, onInputChange, title, subTitle, data, errors }) => {
    ...
    return (
        <Form title={title} subTitle={subTitle} onSubmit={onSubmit} onReset={onReset} onChange={onFormChange} to={`${ROUTES.PROFILE_WALKER}`} >
            ...
            <Input varient='select' error={errors.dogCount} onChange={onInputChange} data={data} name='dogCount' label='כמות כלבים שתוציא בטיול' formText='אנחנו דוגלים במתן דגש על תשומת הלב לכלבים. לא ניתן להוליך מעבר לחמישה כלבים.' options={[{val: 1, text: 'כלב אחד'},{val: 2, text: 'שני כלבים'},{val: 3, text: 'שלושה כלבים'},{val: 4, text: 'ארבעה כלבים'},{val: 5, text: 'חמישה כלבים'}]} errMsg='יש לבחור כמות כלבים.' />
            ...
            <Input varient='select' error={errors.experience} onChange={onInputChange} data={data} name='experience' label='האם יש לך נסיון עם כלבים?' options={[{val: 'base', text:'בסיסי'},{val: 'verified', text:'בעל תעודת אילוף'},{val: 'no', text:'אין'},{val: 'other', text:'אחר'}]} errMsg='יש לבחור אחת מהאפשרויות.' />
            <Input varient='select' error={errors.bigDogs} onChange={onInputChange} data={data} name='bigDogs' label='האם אתה מפחד מכלבים גדולים?' options={[{val: 'yes', text:'כן'},{val: 'no', text:'לא'}]} errMsg='יש לבחור אחת מהאפשרויות.' />
